...
def train_model(model_name: str, task_id: str):
    ...
    try:
        ...
        task_status[task_id] = {"status": "FAILED", "progress": task_status[task_id]["progress"], "error": str(e)}
...
async def start_finetuning(request: FineTuningRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(train_model, "unsloth/Qwen2-VL-2B-Instruct-bnb-4bit", task_id)
...
def save_model_req(request: SaveModelRequest):
    status = task_status.get(request.app_name)  # Note: This assumes app_name is task_id; adjust if needed
