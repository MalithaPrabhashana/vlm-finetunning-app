...
def run_inference(request):
    ...
    inputs = tokenizer(image, input_text, return_tensors="pt").to("cuda" if torch.cuda.is_available() else "cpu")
    output = model.generate(**inputs, streamer=TextStreamer(tokenizer), max_new_tokens=64)
